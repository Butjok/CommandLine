    ...
    public class RuleDependencyChecker
    {
        ...
        private static void CheckDependencies(IList<Tuple<RuleDependencyAttribute, ICustomAttributeProvider>> dependencies, TypeInfo recognizerType)
        {
            ...
            foreach (Tuple<RuleDependencyAttribute, ICustomAttributeProvider> dependency in dependencies)
            {
                ...
                {
                    string message = string.Format("Rule dependency on unknown rule {0}@{1} in {2}", dependency.Item1.Rule, dependency.Item1.Version, dependency.Item1.Recognizer);
                ...
                {
                    string message = string.Format("Rule dependency version mismatch: {0} has maximum dependency version {1} (expected {2}) in {3}", ruleNames[dependency.Item1.Rule], highestRequiredDependency, declaredVersion, dependency.Item1.Recognizer);
                    errors.AppendLine(dependency.Item2.ToString());
                    errors.AppendLine(message);
                }
            }
        ...
        private static int CheckDependencyVersion(StringBuilder errors, Tuple<RuleDependencyAttribute, ICustomAttributeProvider> dependency, string[] ruleNames, int[] ruleVersions, int relatedRule, string relation)
        {
            ...
            {
                string message = string.Format("Rule dependency version mismatch: {0} has version {1} (expected <= {2}) in {3}", path, actualVersion, declaredVersion, dependency.Item1.Recognizer);
                errors.AppendLine(dependency.Item2.ToString());
                errors.AppendLine(message);
            }
        ...
        private static int[] GetRuleVersions(TypeInfo recognizerClass, string[] ruleNames)
        {
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                if (isStatic && isInteger && field.Name.StartsWith("RULE_"))
                {
                    ...
                    {
                        object[] @params = { index, field.Name, recognizerClass.Name };
#if false
                    ...
                    if (ruleMethod == null)
                    {
                        object[] @params = { name, recognizerClass.Name };
#if false
